            ...
            <div class="col-lg-8">
                ...
                <div class="mb-4">
                    <div class="d-flex align-items-center mb-3">
                        ...
                        <div>
                            <h2 class="h3 fw-bold mb-0">Edit Pengajuan</h2>
                            <p class="text-muted small mb-0">{{ $submission->submission_number }} - Ubah data pengajuan sesuai kebutuhan</p>
                        </div>
                    </div>
                </div>
                ...
                @if($submission->status === 'draft')
                    <div class="alert alert-info alert-dismissible fade show mb-4" role="alert">
                        ...
                        <strong>Status Draft</strong> - Anda masih bisa mengubah pengajuan ini. Setelah disubmit, sebagian field tidak bisa diedit lagi.
                        <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                    </div>
                @elseif($submission->status === 'pending')
                    <div class="alert alert-warning alert-dismissible fade show mb-4" role="alert">
                        <i class="bi bi-hourglass-split me-2"></i>
                        <strong>Status Pending</strong> - Pengajuan sedang dalam proses. Perubahan terbatas pada deskripsi saja.
                        <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                    </div>
                @endif
                ...
                <div class="card border-0 shadow-lg">
                    <div class="card-body p-4">
                        <form id="submissionForm" action="{{ route('submissions.update', $submission) }}" method="POST" enctype="multipart/form-data" novalidate>
                            @csrf
                            @method('PUT')
                            ...
                            <div class="form-section mb-4">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="category_id" id="category_id" class="form-select form-select-lg @error('category_id') is-invalid @enderror" required {{ $submission->status !== 'draft' ? 'disabled' : '' }}>
                                    ...
                                    @if($submission->status !== 'draft')
                                        <small class="text-muted d-block mt-2">
                                            <i class="bi bi-lock me-1"></i>Field tidak bisa diubah karena sudah dalam proses
                                        </small>
                                    @endif
                                    @error('category_id')
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group input-group-lg">
                                        ...
                                        <input type="number" name="amount" id="amount" class="form-control @error('amount') is-invalid @enderror" 
                                               value="{{ old('amount', (int)$submission->amount) }}" required min="1" step="1000" placeholder="0" {{ $submission->status !== 'draft' ? 'disabled' : '' }}>
                            ...
                            <div class="form-section mb-4">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea name="description" id="description" rows="5" class="form-control @error('description') is-invalid @enderror" 
                                              placeholder="Jelaskan tujuan pengajuan ini secara detail..." required>{{ old('description', $submission->description) }}</textarea>
                                    <div class="d-flex justify-content-between mt-2">
                                        <small class="text-muted">Anda dapat mengubah keterangan kapan saja</small>
                                        <small class="text-muted"><span id="charCount">{{ strlen($submission->description) }}</span>/500</small>
                                    </div>
                                    @error('description')
                                        <div class="invalid-feedback d-block">
                                            <i class="bi bi-exclamation-circle me-1"></i>{{ $message }}
                            ...
                            <div class="form-section mb-4">
                                            ...
                                            @foreach($submission->attachment as $index => $path)
                                                <div class="alert alert-info mb-0">
                                                    <div class="d-flex justify-content-between align-items-center">
                                                        ...
                                                        <a href="{{ route('submissions.download', ['submission' => $submission->id, 'index' => $index]) }}" class="btn btn-sm btn-outline-info" target="_blank">
                                ...
                                <div class="mb-3">
                                    <label for="attachment" class="form-label fw-bold">
                                        <i class="bi bi-paperclip me-2 text-warning"></i>Ganti Berkas Pendukung
                                    </label>
                                    <div class="file-upload-wrapper">
                                        ...
                                        <div class="upload-area border-2 border-dashed rounded-3 p-5 text-center cursor-pointer" id="uploadArea">
                                            ...
                                            <p class="fw-bold mb-1">Klik atau Drag & Drop File Baru</p>
                                    ...
                                    <small class="text-muted d-block mt-2">
                                        <i class="bi bi-info-circle me-1"></i>Biarkan kosong jika tidak ingin mengubah lampiran
                                    </small>
                                    @error('attachment')
                                        <div class="invalid-feedback d-block mt-2">
                                            <i class="bi bi-exclamation-circle me-1"></i>{{ $message }}
                            ...
                            <div class="card bg-light border-0 mb-4">
                                <div class="card-body">
                                    <h6 class="fw-bold mb-3">
                                        <i class="bi bi-clipboard-check me-2 text-success"></i>Ringkasan Perubahan
                                    </h6>
                                    <div class="row g-3">
                                        <div class="col-md-6">
                                            <small class="text-muted d-block">Kategori</small>
                                            <strong id="summaryCategory" class="text-primary">{{ $submission->category->name }}</strong>
                                        </div>
                                        <div class="col-md-6">
                                            <small class="text-muted d-block">Nominal</small>
                                            <strong id="summaryAmount" class="text-success">Rp {{ number_format($submission->amount, 0, ',', '.') }}</strong>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="d-flex gap-2 mt-5">
                                ...
                                @endif
                                <a href="{{ route('submissions.show', $submission) }}" class="btn btn-outline-secondary btn-lg">
                ...
                <div class="alert alert-warning mt-4 border-0">
                    <div class="d-flex">
                        ...
                        <div>
                            ...
                            <ul class="mb-0 mt-2">
                                @if($submission->status !== 'draft')
                                    <li>Kategori dan nominal tidak bisa diubah karena pengajuan sudah dalam proses</li>
                                @endif
                                <li>Keterangan dapat diubah kapan saja</li>
                                <li>Setiap perubahan akan tercatat dalam sistem</li>
                            </ul>
                        </div>
                    </div>
                </div>
